...
from src.helper.helper import select_with_like
...
class TestCompareCvDb:
    ...
    def test_select_with_like(self, setup_db):
        ...
        db.salva_registro("positions", "url, description", "'URL', '{}'".format(p1))
        db.salva_registro("positions", "url, description", "'URL', '{}'".format(p2))
        db.salva_registro("positions", "url, description", "'URL', '{}'".format(p3))
