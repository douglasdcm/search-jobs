...
from src.helper.helper import data_pre_processing_portuguese, notify_by_email, validate_email, truncate_message
...
class TestUnitHelper:
    ...
    def test_truncate_text_limits_it(self):
        txt = """
            ...
            bbbbbbbbbb
            """
        actual = len(truncate_message(txt))
        assert actual == LIMITE

    def test_not_truncate_text_smaller_limit(self):
        txt = "aaaaaaaaa"
        actual = len(truncate_message(txt))
        assert actual == len(txt)